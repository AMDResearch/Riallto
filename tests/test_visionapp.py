...
from .test_callgraphs import _test_callgraph_2in1out_singlekernel_build
...
from .test_callgraphs import _test_callgraph_3in1outNrtps_singlekernel_build
from .test_applications import clear_kernel_cache
from .test_kernels_build import _test_kernel_build
...
multiple_rtp = [[PlusN, 1], [InRange, 2], [ThresholdRgba, 9],
                ...
                [Filter2d(linewidth=1920), 8], [Filter2dScalar(linewidth=1280), 8], 
                [Filter2dScalar(linewidth=1920), 8] ]

@pytest.mark.parametrize('kernel', one_rtp)
def test_callgraph_kernel_one_rtp(clear_kernel_cache, kernel):
...
def test_callgraph_addweighted_build(clear_kernel_cache, kernel):
...
def test_callgraph_twobuffin_onebuffout_build(clear_kernel_cache, kernel):
...
def test_callgraph_multiplertp_build(clear_kernel_cache, kernel):
    """ Build test of callgraph kernels with multiple RTP """

    _test_callgraph_1in1outNrtps_singlekernel_build(kernel[0], kernel[1])
